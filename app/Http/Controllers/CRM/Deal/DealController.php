<?php
    ...
    class DealController extends Controller
    {
        ...
        public function dealsByOwner($owner_id = null)
        {
            return $this->service->getDealsByOwner($owner_id);
        }

        public function dealDetailsById($id = null)
        {
            return $this->service->dealDetails($id);
        }
        ...
        public function update(Request $request, Deal $deal)
        {
            ...
            $this->service->notifyAndMailToOwner($deal, 'deal_updated');
        ...
        public function destroy(Deal $deal)
        {
            ...
            $this->service->notifyAndMailToOwner($deal, 'deal_deleted');
        ...
        public function dealActivitiesSync(\Illuminate\Http\Request $request, Deal $deal)
        {
            ...
            $activity = $deal->activity()->create($options);
        ...
        {
            $deal->tags()->attach($request->tag_id);
            $this->updateRevisionForTagAttachOrDetachOnDeal(
                $deal,
                $request->tag_id,
                'tag_added'
            );
        ...
        public function updateRevisionForTagAttachOrDetachOnDeal(
            Deal $deal,
            $tagId,
            $action='tag_added'
        ) {
        ...
        {
            $deal->tags()->detach($request->tag_id);
            $this->updateRevisionForTagAttachOrDetachOnDeal(
                $deal,
                $request->tag_id,
                'tag_removed'
        ...
        public function dealProposals(Deal $deal)
        {
            return $this->service->proposalWithDeal($deal);
        }
        ...
        public function tagsMultipleMarkedDeal()
        {
            ...
            foreach ($deals as $deal) {
                try {
                    ...
                    $this->updateRevisionForTagAttachOrDetachOnDeal($deal, $tagId, 'tag_added');
        ...
        public function dettachMultipleDealTag()
        {
            ...
            foreach ($deals as $deal) {
                try {
                    ...
                    $this->updateRevisionForTagAttachOrDetachOnDeal($deal, $tagId,'tag_removed');
                } catch (\Illuminate\Database\QueryException $e) {
                }
            }
            return updated_responses('deal');
        ...
        public function revision(Deal $deal)
        {
            ...
            foreach ($dirtyData as $history) {
                $data = [];
                $data['responsible_user'] = User::with(['profilePicture', 'profile','roles'])->find($history->user_id);
                if ($history->key == 'stage_id') {
                    ...
                    if (isset($newValue->id)) {
                        ...
                        $data['new_value'] = $newValue->lost_reason . ($history->old_value ? (' - ' . $history->old_value) : '');
                    }
