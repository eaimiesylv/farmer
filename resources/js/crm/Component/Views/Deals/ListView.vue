...
export default {
    ...
    return {
      ...
      commonColumn: [
        ...
        },
        {
          ...
          modifier: (value, row) => {
            return value.length
              ? value[0].attach_login_user_id == window.user.id
                ? window.user.full_name
                : value[0].name
        ...
        },
        {
          ...
          key: "invoice",
          isVisible:
            this.$can("update_deals") || this.$can("delete_deals") ? true : false,
        },
      ],
      options: {
        ...
        showAction: true,
        showManageColumn:
          !this.$can("manage_public_access") && this.$can("client_access") ? false : true,
        orderBy: "desc",
        actionType: "default",
        actions: [
          {
  ...
  methods: {
    filteredValues(value) {
      if (value["pipeline"]) {
        this.pipelineId =
          value["pipeline"] === "all" ? this.initialPipeline : value.pipeline;
        this.currentPipeline = this.initialPipeline
          ? this.options.filters
              .find((el) => el.title === this.$t("select_a_pipeline"))
              .option.find((el) => el.id == value.pipeline).value
    ...
    getDealValue() {
      this.axiosGet(route("deal.value")).then((response) => {
        let dealValueFilter = this.options.filters.find(
          (item) => item.key === "deal-value"
        );
    ...
    setPipelineFilterInit() {
      let pipeline = this.options.filters.find(
        (item) => item.title === this.$t("select_a_pipeline")
      );
    ...
    getAction(rowData, actionObj, active) {
      if (actionObj.title == this.$t("edit")) {
        ...
        this.selectedDeal.lead_type =
          rowData.contextable_type == "App\\Models\\CRM\\Organization\\Organization"
            ? 2
            : 1;
